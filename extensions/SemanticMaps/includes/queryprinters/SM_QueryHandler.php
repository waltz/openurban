...
class SMQueryHandler {
	...
	protected function handleResultRow( array /* of SMWResultArray */ $row ) {
		...
		foreach ( $row as $i => $resultArray ) {
			...
			while ( ( /* SMWDataValue */ $dataValue = $resultArray->getNextDataValue() ) !== false ) {
				...
				if ( $dataValue->getTypeID() == '_wpg' && $i == 0 ) {
					...
					$geoShapes[ $polyHandler->getGeoType() ][] = $polyHandler->shapeFromText();
				}
		...
		$this->addMetadata( $geoShapes, $text, $icon, $properties, Title::newFromText( $title ) );
		$this->geoShapes = array_merge_recursive( $this->geoShapes, $geoShapes );
	}
