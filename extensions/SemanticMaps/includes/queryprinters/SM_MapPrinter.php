...
class SMMapPrinter extends SMWResultPrinter {
	...
	protected function handleMarkerData( array &$params, $queryHandler ) {
		...
		foreach ( $queryShapes as $type => $list ) {
			foreach( $list as $object ) {
				if( $type == 'locations' ) {
					if ( $object->isValid() ) {
						$jsonObj = $object->getJSONObject( $params['title'], $params['label'], $iconUrl, '', '', $visitedIconUrl );
					...
					$jsonObj = $object->getJSONObject( $params['title'], $params['label'] );
					$params[$type][] = $jsonObj;
				}
			}
		}
