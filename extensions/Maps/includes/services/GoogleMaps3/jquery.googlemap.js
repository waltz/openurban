	...
	$.fn.googlemaps = function (options) {
			...
			if ( options.centre === false || options.zoom === false ) {
				...
				if ( _this.rectangles.length > 0 ) {
					for ( var i = _this.rectangles.length - 1; i >= 0; i-- ) {
						bounds.extend( _this.rectangles[i].getBounds().getCenter() );
					}
				}
				return bounds;
			}
		...
		this.setup = function () {
			...
			if (options.centre === false) {
				if ( options.locations.length > 1 || options.rectangles.length > 0 || options.circles.length > 0 || options.lines.length > 0 || options.polygons.length > 0 ) {
